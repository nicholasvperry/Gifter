    ...
    public class PostRepository : BaseRepository, IPostRepository
    {
        ...
        public List<Post> GetAllWithComments()
        {
            using (var conn = Connection)
            {
                ...
                SELECT p.Id AS PostId, p.Title, p.Caption, p.DateCreated AS PostDateCreated,
                       ...
                       up.Id as UserProfileId, up.Name as PostUserName, up.Bio, up.Email, up.DateCreated AS UserProfileDateCreated,
                       up.ImageUrl AS UserProfileImageUrl,
                        ...
                        if (existingPost == null)
                        {
                            existingPost = new Post()
                            {
                                ...
                                UserProfile = new UserProfile()
                                {
                                    ...
                                    Name = DbUtils.GetString(reader, "PostUserName"),
                                    Email = DbUtils.GetString(reader, "Email"),
                                    DateCreated = DbUtils.GetDateTime(reader, "UserProfileDateCreated"),
                                    ImageUrl = DbUtils.GetString(reader, "UserProfileImageUrl"),
                                },
        ...
        public Post GetById(int id)
        {
            using (var conn = Connection)
            {
                ...
                {
                    cmd.CommandText = @"
                           SELECT p.Id, p.Title, p.Caption, p.DateCreated AS PostDateCreated, p.ImageUrl, p.UserProfileId,
                            up.Name, up.Bio, up.Email, up.DateCreated AS UserProfileDateCreated,
                            up.ImageUrl AS UserProfileImageUrl
                            FROM Post p
                            LEFT JOIN UserProfile up ON p.UserProfileId = up.id
        ...
        //----------Using string from dbutils for SQL Query
        //there needs to be @$. The @ lets us use more than one line. The $ lets us pull value from dbutils
            ...
            using (var conn = Connection)
            {
                ...
                using (var cmd = conn.CreateCommand())
                {
                    cmd.CommandText = @$"
                       ...
                       up.Name AS PostUserName, up.Bio, up.Email, up.DateCreated AS UserProfileDateCreated,
                       up.ImageUrl AS UserProfileImageUrl,
                  ...
                  FROM Post p
                    ...
                    //needs to be while to keep getting comments
                    while (reader.Read())
                    {
                        //needs if statement to make only one post list
                        if (post == null)
                        {
                            post = new Post()
                            {
                        ...
                        //Checks to see if (we have comments vefore we mess with them
                        if (DbUtils.IsNotDbNull(reader, "CommentId"))
                        {
        ...
        public List<Post> Hottest(DateTime hottest)
        {
            using (var conn = Connection)
            {
                ...
                using (var cmd = conn.CreateCommand())
                {
                    ...
                    FROM Post p 
                        LEFT JOIN UserProfile up ON p.UserProfileId = up.id
                    WHERE p.DateCreated >= @hottest  ";

                    

                    cmd.CommandText = sql;
                    DbUtils.AddParameter(cmd, "@hottest", hottest);
